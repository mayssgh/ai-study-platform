  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 28 }}>
        ...
        <StatCard label="Active Courses" value={stats?.totalCourses ?? "—"} icon="📚" color="#a78bfa" loading={statsLoading} />
      ...
      <div style={{ background: "linear-gradient(135deg, #1a1f2e 0%, #141824 100%)", border: "1px solid #2a2f42", borderRadius: 16, padding: 24 }}>
        ...
        {selected && (
          <div style={{
            ...
            background: "#0d1117", border: "1px solid #2a3f6a",
          }}>
...
const ComingSoon = ({ title, icon }) => (
  <>
    ...
    <div style={{
      background: "linear-gradient(135deg, #1a1f2e 0%, #141824 100%)",
      border: "1px solid #2a2f42", borderRadius: 16, padding: "80px 24px", textAlign: "center",
    }}>
      <div style={{ fontSize: 64, marginBottom: 20 }}>{icon}</div>
      <div style={{ color: "#f1f5f9", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 22, marginBottom: 8 }}>Coming Soon</div>
      <div style={{ color: "#4b5563", fontSize: 14 }}>This page is being built. Check back soon!</div>
    </div>
  </>
);
...
export default function Dashboard() {
  ...
  const renderPage = () => {
    switch (activeNav) {
      ...
      case "courses":   return <ComingSoon title="Courses" icon="📚" />;
      case "analytics": return <ComingSoon title="Analytics" icon="📊" />;
